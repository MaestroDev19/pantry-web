...
function isHouseholdJoinSuccessBody(data: unknown): data is HouseholdJoinSuccessBody {
	if (!data || typeof data !== 'object') return false
	const o = data as Record<string, unknown>
...
export async function createPersonalHousehold(
	...
	const parsed = parsePantryApiErrorMessage(body) ?? (typeof body === 'string' ? body : null)
...
function isLeaveHouseholdSuccessBody(data: unknown): data is LeaveHouseholdSuccessBody {
	if (!data || typeof data !== 'object') return false
	const o = data as Record<string, unknown>
	const idOk =
...
export async function convertHouseholdToJoinable(
	...
	const msg = parsePantryApiErrorMessage(parsedBody) ?? (typeof parsedBody === 'string' ? parsedBody : null)
	const fallback =
		res.status === 429
