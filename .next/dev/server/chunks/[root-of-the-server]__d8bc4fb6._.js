...
function clearAuthCookiesAndRedirect(request, pathname = "/") {
...
async function updateSession(request) {
    ...
    try {
        ...
        const isInvalidRefreshToken = err && typeof err === "object" && "code" in err && err.code === "refresh_token_not_found";
        if (isInvalidRefreshToken) {
            return clearAuthCookiesAndRedirect(request);
        }
        throw err;
    }
    const isAuthPath = request.nextUrl.pathname.startsWith("/login") || request.nextUrl.pathname.startsWith("/auth") || request.nextUrl.pathname === "/";
    if (!user && !isAuthPath) {
        return clearAuthCookiesAndRedirect(request);
    }
